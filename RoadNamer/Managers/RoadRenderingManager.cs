...
namespace RoadNamer.Managers
{
    class RoadRenderingManager : SimulationManagerBase<RoadRenderingManager, DistrictProperties>, IRenderableManager, ISimulationManager
    {
        ...
        protected override void BeginOverlayImpl(RenderManager.CameraInfo cameraInfo)
        {
            ...
            {
                Debug.Log(districtManager.m_properties.m_areaNameFont.size.ToString());
                ...
                if (lastCount != RoadNameManager.m_roadNames.Count)
                {
                    lastCount = RoadNameManager.m_roadNames.Count;
                    ...
                    try
                    {
                        ...
                        foreach (KeyValuePair<ushort, string> road in RoadNameManager.m_roadNames)
                        {
                            if (road.Key != 0)
                            {
                                string roadName = road.Value;
                                ...
                                if (roadName != null)
                                {
                                    ...
                                    NetSegment netSegment = netManager.m_segments.m_buffer[road.Key];
